...
def run_command(args, expected_exit_code=0):
    ...
    print(
        Colorizer.cyan(" (exit code %s, expected %s)" % (exitcode, expected_exit_code))
    )
    ...
    while True:
        ...
        if state in {"running", "ready", "failed"}:
            assert state == "running", (
                "waiting for 'running' state, but got '%s'" % state
            )
            return
        time.sleep(0.5)
...
def check_contains(true_value, pred_value):
    if isinstance(true_value, list):
        ...
        assert true_value in pred_value or re.search(true_value, pred_value), (
            "expected something that contains '%s', but got '%s'"
...
def temp_instance():
    ...
    run_command(
        [cl, "config", "server/engine_url", "sqlite:///" + remote_home + "/bundle.db"]
    )
...
def test(ctx):
    ...
    check_contains(
        ["bundle_type", "uuid", "owner", "created"], run_command([cl, "info", uuid])
    )
    check_contains("license", run_command([cl, "info", "--raw", uuid]))
    check_contains(
        ["host_worksheets", "contents"], run_command([cl, "info", "--verbose", uuid])
    )
    # test interpret_file_genpath
    check_equals(
        " ".join(test_path_contents("a.txt").splitlines(False)), get_info(uuid, "/")
    )
...
def test(ctx):
    ...
    uuid = run_command(
        [cl, "upload", test_path("broken-symlink"), "--follow-symlinks"], 1
    )
...
def test(ctx):
    ...
    for suffix in [".tar.gz", ".zip"]:
        ...
        check_equals(
            os.path.basename(archive_path).replace(suffix, ""), get_info(uuid, "name")
        )
        check_equals(
            test_path_contents("dir1/f1"), run_command([cl, "cat", uuid + "/f1"])
        )
...
def test(ctx):
    ...
    run_command(
        [cl, "download", uuid + "/a-symlink.txt", "-o", path], 1
    )  # Disallow symlinks
...
def test(ctx):
    ...
    run_command(
        [cl, "wedit", wuuid, "--title", u"fáncy ünicode"]
    )  # try unicode in worksheet title
    run_command(
...
def test(ctx):
    ...
    check_contains(
        ["Switched", group_wname, group_wuuid], run_command([cl, "work", group_wuuid])
    )
    ...
    check_contains(
        group_wuuid[:8], run_command([cl, "wls", "group={}".format(group_uuid)])
    )
    check_contains(
        group_wuuid[:8], run_command([cl, "wls", "group={}".format(group_name)])
    )
...
def test(ctx):
    ...
    check_equals(uuid1, run_command([cl, "search", "uuid=" + uuid1[0:8] + "%", "-u"]))
    check_equals(
        uuid1, run_command([cl, "search", "uuid=" + uuid1, "name=" + name, "-u"])
    )
    check_equals(
        uuid1 + "\n" + uuid2,
    ...
    check_contains(
        group_buuid[:8], run_command([cl, "search", "group={}".format(group_uuid)])
    )
    check_contains(
        group_buuid[:8], run_command([cl, "search", "group={}".format(group_name)])
    )
...
def test(ctx):
    ...
    dependent = run_command(
        [cl, "run", ":%s" % special_name, "cat %s/stdout" % special_name]
    )
    wait(dependent)
    ...
    check_contains(
        ["Switched", new_wname, new_wuuid], run_command([cl, "work", new_wuuid])
    )
    ...
    check_equals(
        sugared_remote_uuid, run_command([cl, "search", sugared_remote_name, "-u"])
    )
...
def test(ctx):
    ...
    for running in [True, False]:
        ...
        assert (
            "5\n6\n7" not in info_output
        ), "info output should contain only first 10 lines"
...
def test(ctx):
    ...
    input_uuid = run_command(
        [cl, "upload", test_path("a.txt"), "-n", simple_name + "-in1"]
    )
...
def test(ctx):
    ...
    uuid = run_command(
        [cl, "run", "--request-docker-image=codalab/ubuntu:1.9", "echo $SCALA_HOME"]
    )
    wait(uuid)
    check_equals("/opt/scala/current", run_command([cl, "cat", uuid + "/stdout"]))
    uuid = run_command(
        [cl, "run", "--request-docker-image=codalab/torch:1.1", "echo $LUA_PATH"]
    )
    wait(uuid)
    check_contains(
