...
class BundleManagerMockedManagerTest(unittest.TestCase):
    ...
    def test_filter_and_sort_workers_tag_exclusive_priority(self):
        ...
        self.bundle_resources.tag = "tag=worker_X"
        sorted_workers_list = self.bundle_manager._filter_and_sort_workers(
            self.workers_list, self.bundle, self.bundle_resources
        )
        self.assertEqual(len(sorted_workers_list), 2)
    ...
    def test_get_dominating_workers(self):
        dominating_workers = self.bundle_manager._get_dominating_workers(
            self.bundle_resources, self.workers_list
        )
        self.assertEqual(len(dominating_workers), 6)

    def test_get_dominating_workers_with_tag(self):
        self.bundle_resources.tag = 'tag=worker_X'
        dominating_workers = self.bundle_manager._get_dominating_workers(
            self.bundle_resources, self.workers_list
        )
        self.assertEqual(len(dominating_workers), 2)
        for worker in dominating_workers:
            self.assertEqual(worker['tag'], 'worker_X')

    def test_get_dominating_workers_with_bad_formatted_tag(self):
        self.bundle_resources.tag = 'tag='
        dominating_workers = self.bundle_manager._get_dominating_workers(
            self.bundle_resources, self.workers_list
        )
        self.assertEqual(len(dominating_workers), 0)

    def test_get_dominating_workers_without_tag_prefix(self):
        self.bundle_resources.tag = 'worker_X'
        dominating_workers = self.bundle_manager._get_dominating_workers(
            self.bundle_resources, self.workers_list
        )
        self.assertEqual(len(dominating_workers), 2)
    ...
    def test_get_dominating_workers_non_existant_tag(self):
        self.bundle_resources.tag = 'worker_Y'
        dominating_workers = self.bundle_manager._get_dominating_workers(
            self.bundle_resources, self.workers_list
        )
        self.assertEqual(len(dominating_workers), 0)
    ...
    def test_get_dominating_workers_none_exist(self):
        self.bundle_resources.cpus = 10000000  # no workers dominate this
        dominating_workers = self.bundle_manager._get_dominating_workers(
            self.bundle_resources, self.workers_list
        )
        self.assertEqual(len(dominating_workers), 0)
    ...
    def test_get_resource_recommendations_multiple_workers(self):
        self.bundle_resources.tag = 'test-queue'
        self.bundle_resources.cpus = 2
        self.bundle_resources.gpus = 3
        self.bundle_resources.memory = 800000
        self.bundle_resources.disk = 900000
        workers_list = [
            self.workers_list[0],
            self.workers_list[1],
            self.workers_list[2],
        ...
        recommendations = self.bundle_manager._get_resource_recommendations(
            self.bundle_resources, workers_list
        )
        expected = (
            'Available resources: [No queue (test-queue requested), 2 GPUs (3 '
            'requested), 3.9k memory (781k requested), 3.9k disk (878k requested)], '
            '[No queue (test-queue requested), 1 GPUs (3 requested), 3.9k memory '
            '(781k requested), 3.9k disk (878k requested)], [No queue (test-queue '
    ...
    def test_worker_to_run_resources(self):
        ...
        self.assertEqual(worker['tag'], worker_resources.tag)
