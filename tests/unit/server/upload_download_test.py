...
class BaseUploadDownloadBundleTest(TestBase):
    ...
    def check_file_target_contents(self, target):
        ...
        with self.assertRaises(tarfile.ReadError):
            with tarfile.open(fileobj=self.download_manager.stream_tarred_gzipped_directory(target), mode='r:gz') as f:
                pass
        
        with BytesIO(self.download_manager.read_file_section(target, offset=3, length=4, gzipped=False)) as f:
            self.assertEqual(f.read(), b"lo w")

        with gzip.GzipFile(fileobj=BytesIO(self.download_manager.read_file_section(target, offset=3, length=4, gzipped=True))) as f:
            self.assertEqual(f.read(), b"lo w")
        
        with BytesIO(self.download_manager.summarize_file(target, num_head_lines=1, num_tail_lines=1, max_line_length=3, truncation_text="....", gzipped=False)) as f:
    ...
    def check_folder_target_contents(self, target, expected_members=[]):
        ...
        with self.assertRaises(IsADirectoryError):
            self.download_manager.summarize_file(target, num_head_lines=1, num_tail_lines=1, max_line_length=3, truncation_text="....", gzipped=False)
    ...
    def test_bundle_folder(self):
        ...
        bundle = self.create_run_bundle()
        self.save_bundle(bundle)
        self.upload_folder(bundle, [
            ("item.txt", b"hello world"),
        ...
        self.assertEqual(str(info["resolved_target"]), f"{bundle.uuid}:")
        self.assertEqual(info["contents"], [{'name': 'item.txt', 'size': 11, 'perm': 420, 'type': 'file'}, {'name': 'src', 'size': 96, 'perm': 493, 'type': 'directory', 'contents': [{'name': 'item2.txt', 'size': 11, 'perm': 420, 'type': 'file'}]}])
        self.check_folder_target_contents(target, expected_members=['.', './item.txt', './src', './src/item2.txt'])
        ...
        self.assertEqual(info["contents"], [{'name': 'item2.txt', 'size': 11, 'perm': 420, 'type': 'file'}])
...
class RegularBundleStoreTest(BaseUploadDownloadBundleTest, unittest.TestCase):
    ...
    def upload_file(self, bundle, contents):
        sources = [["contents.txt", BytesIO(contents)]]
        self.upload_manager.upload_to_bundle_store(
            bundle,
            sources,
            follow_symlinks=False,
...
#             simplify_archives=True
#         )
# class ABGetTargetInfoTest(GetTargetInfoTest):
#     def create_run_bundle(self):
#         bundle = super().create_run_bundle()
#         bundle.metadata.set_metadata_key("link_url", f"azfs://storageclwsdev0/bundles/{bundle.uuid}/contents.zip")
#         return bundle