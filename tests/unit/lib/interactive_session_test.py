...
class InteractiveSessionTest(unittest.TestCase):
    def test_get_docker_run_command(self):
        ...
        expected_regex = (
            r'docker run -it --name interactive-session-0x[a-z0-9]{32} -w \/0x[a-z0-9]{32} -v '
            r'[\s\S]{0,100}local\/path1:\/0x[a-z0-9]{32}\/key:ro -v [\s\S]{0,100}local\/path2:\/0x[a-z0-9]{32}\/key2:ro '
    ...
    def test_get_docker_run_command_with_subpaths(self):
        ...
        expected_regex = (
            r'docker run -it --name interactive-session-0x[a-z0-9]{32} -w \/0x[a-z0-9]{32} -v '
            r'[\s\S]{0,100}local\/path1/sub/path1:\/0x[a-z0-9]{32}\/key:ro -v [\s\S]{0,100}local\/path2/sub/path2'
            r':\/0x[a-z0-9]{32}\/key2:ro some-docker-image bash'
        )
        self.assertTrue(re.match(expected_regex, session.get_docker_run_command()))
