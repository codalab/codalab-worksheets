...
def canonicalize(metadata_dicts):
  ...
  '''
    ...
    return sorted(
        sorted(
            (k, v) for (k, v) in dict(metadata_dict).iteritems() if k != 'id'
...
class MockBundle(object):
            ...
            for (field, value) in self._fields.iteritems():
                if field == 'metadata':
                    ...
                    expected_value = canonicalize(
                        metadata_to_dicts(
                            self._fields['uuid'], self._fields['metadata']
                        )
                    )
    ...
    def to_dict(self, strict=None):
        ...
        result['metadata'] = metadata_to_dicts(
            result['uuid'], result['metadata']
        )
        return result
...
class BundleModelTest(unittest.TestCase):
    ...
    def test_save_and_get_bundle(self):
        ...
        with mock.patch(
            get_bundle_subclass_path, lambda bundle_type: MockBundle
        ):
