...
class CLIUtilTest(unittest.TestCase):
    ...
    def test_parse_target_spec(self):
        cases = [
            (
                'https://worksheets.codalab.org::some-worksheet//some-bundle-2.dirname/this/is/a/path.txt',
                (
                    'https://worksheets.codalab.org', 'some-worksheet',
                    'some-bundle-2.dirname', 'this/is/a/path.txt'
                )
            ), (
                'some-worksheet//some-bundle-2.dirname/this/is/a/path.txt', (
                    None, 'some-worksheet', 'some-bundle-2.dirname',
                    'this/is/a/path.txt'
                )
            ), (
                'some-bundle-2.dirname/this/is/a/path.txt',
    ...
    def test_desugar(self):
        self.assertEqual(
            cli_util.desugar_command([], 'echo hello'), ([], 'echo hello')
        )
        self.assertEqual(
        ...
        self.assertEqual(
            cli_util.desugar_command(['a:b'], 'echo %b:c%'),
            (['a:b', 'b:c'], 'echo b')
        )
        self.assertEqual(
            cli_util.desugar_command(['a:b'], 'echo %c%'),
            (['a:b', 'b2:c'], 'echo b2')
        )
        self.assertEqual(
            cli_util.desugar_command(['a:b'], 'echo %:c%'),
        ...
        self.assertEqual(
            cli_util.desugar_command(['a:b'], 'echo %a:b% %a:b%'),
            (['a:b'], 'echo a a')
        )
        self.assertEqual(
            cli_util.desugar_command([], 'echo %a% %a% %a%'),
            (['b1:a'], 'echo b1 b1 b1')
        )
        self.assertRaises(
            UsageError,
            lambda: cli_util.desugar_command([], 'echo %a:b% %a:c%')
        )
        self.assertRaises(
            UsageError, lambda: cli_util.desugar_command([':b'], 'echo %b:c%')
        )
