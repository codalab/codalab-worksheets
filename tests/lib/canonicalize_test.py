...
class CanonicalizeTest(unittest.TestCase):
    def test_get_bundle_uuid(self):
        ...
        bundle_counts = {missing_name: 0, unique_name: 1, ambiguous_name: 2}
        ...
        class MockBundleModel(object):
            def get_bundle_uuids(self, conditions, max_results, count=False):
                tester.assertEqual(
                    set(conditions), set(["name", "user_id", "worksheet_uuid"])
                )
                name = conditions["name"].replace("%", "")
                tester.assertIn(name, bundle_counts)
                count = bundle_counts[name]
                return [test_uuids[i] for i in range(count)]
        ...
        self.assertEqual(
            uuid, canonicalize.get_bundle_uuid(model, user, worksheet_uuid, uuid)
        )
        # Test that get_bundle_uuid returns the uuid of a uniquely named bundle.
        self.assertEqual(
            test_uuid,
            canonicalize.get_bundle_uuid(model, user, worksheet_uuid, unique_name),
        )
        # Test that get_bundle_uuid raises UsageErrors on missing names.
        self.assertRaises(
            UsageError,
            lambda: canonicalize.get_bundle_uuid(
                model, user, worksheet_uuid, missing_name
            ),
        )
        ...
        self.assertRaises(
            UsageError,
            lambda: canonicalize.get_bundle_uuid(
                model, user, worksheet_uuid, "names have no exclamations!"
            ),
        )
