...
def upgrade():
    ...
    op.add_column(
        "worker_dependency", sa.Column("dependencies", MEDIUMBLOB(), nullable=False)
    )
    ...
    if results:
        dependencies = {}
        for row in results:
            dependencies.setdefault((row.user_id, row.worker_id), []).append(
                (row.dependency_uuid, row.dependency_path)
            )
        op.execute("DELETE FROM `worker_dependency`;")
        op.execute(
            worker_dependency.insert().values(
                [
                    {
                        "user_id": user_id,
                        "worker_id": worker_id,
    ...
    op.execute(
        "ALTER TABLE `worker_dependency` ADD PRIMARY KEY (`user_id`, `worker_id`);"
    )
...
def downgrade():
    ...
    op.drop_constraint(
        "worker_dependency_ibfk_1", "worker_dependency", type_="foreignkey"
    )
    op.drop_constraint(
        "worker_dependency_ibfk_2", "worker_dependency", type_="foreignkey"
    )
    op.execute("ALTER TABLE `worker_dependency` DROP PRIMARY KEY;")
    op.create_foreign_key(
        "worker_dependency_ibfk_1",
        "worker_dependency",
        "worker",
        ["user_id", "worker_id"],
        ["user_id", "worker_id"],
    ...
    op.add_column(
        "worker_dependency", sa.Column("dependency_uuid", sa.String(63), nullable=False)
    )
    op.add_column(
        "worker_dependency", sa.Column("dependency_path", sa.Text(), nullable=False)
    )
