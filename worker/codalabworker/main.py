...
chmod 600 %s""" % args.password_file
    ...
    def create_local_run_manager(worker):
        ...
        dependency_manager = LocalFileSystemDependencyManager(
            ...
            bundle_service, args.work_dir, max_work_dir_size_bytes,
            args.max_dependencies_serialized_length
        )

        image_manager = DockerImageManager(
            docker_client, os.path.join(args.work_dir, 'images-state.json'),
            max_images_bytes
        )

        return LocalRunManager(
            worker, docker_client, image_manager, dependency_manager,
            os.path.join(args.work_dir, 'run-state.json'), cpuset, gpuset,
            args.work_dir, args.network_prefix
        )

    worker = Worker(
        create_local_run_manager,
        os.path.join(args.work_dir, 'worker-state.json'), args.id, args.tag,
