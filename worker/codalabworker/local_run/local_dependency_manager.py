...
class LocalFileSystemDependencyManager(StateTransitioner, BaseDependencyManager):
    ...
    def __init__(
        ...
        if not os.path.exists(self.dependencies_dir):
            ...
            os.makedirs(self.dependencies_dir, mode=0o770)
        ...
        self._downloading = ThreadDict(
            fields={"success": False, "failure_message": None}
        )
        self._load_state()
    ...
    def _save_state(self):
        with self._lock:
            self._state_committer.commit(
                {"dependencies": self._dependencies, "paths": self._paths}
            )
