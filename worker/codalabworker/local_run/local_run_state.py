...
LocalRunState = namedtuple(
    'RunState', [
        'stage', 'run_status', 'bundle', 'bundle_path', 'resources',
        'start_time', 'container_id', 'docker_image', 'is_killed',
        'has_contents', 'cpuset', 'gpuset', 'time_used', 'max_memory',
        'disk_utilization', 'info'
    ]
)
...
class LocalRunStateMachine(StateTransitioner):
    ...
    def __init__(self, run_manager):
        ...
        self.add_transition(
            LocalRunStage.PREPARING, self._transition_from_PREPARING
        )
        self.add_transition(
            LocalRunStage.RUNNING, self._transition_from_RUNNING
        )
        self.add_transition(
            LocalRunStage.CLEANING_UP, self._transition_from_CLEANING_UP
        )
        self.add_transition(
            LocalRunStage.UPLOADING_RESULTS,
            self._transition_from_UPLOADING_RESULTS
        )
        self.add_transition(
            LocalRunStage.FINALIZING, self._transition_from_FINALIZING
        )
    ...
    def _transition_from_PREPARING(self, run_state):
        ...
        if run_state.is_killed:
            return run_state._replace(
                stage=LocalRunStage.CLEANING_UP, container_id=None
            )
        ...
        for dep in run_state.bundle['dependencies']:
            ...
            if dependency_state.stage == DependencyStage.DOWNLOADING:
                ...
                return run_state._replace(
                    stage=LocalRunStage.CLEANING_UP, info=run_state.info
                )
        ...
        if image_state.stage == DependencyStage.DOWNLOADING:
            status_messages.append(
                ...
                (image_state.message or docker_image)
            )
            dependencies_ready = False
        ...
        for dep in run_state.bundle['dependencies']:
            ...
            if not child_path.startswith(run_state.bundle_path):
                raise Exception(
                    'Invalid key for dependency: %s' % (dep['child_path'])
                )
            ...
            dependency_path = os.path.join(
                self._run_manager.dependencies_dir, dependency_path
            )
        ...
        digest = self._run_manager.docker.get_image_repo_digest(
    ...
    def _transition_from_RUNNING(self, run_state):
        ...
        def check_resource_utilization(run_state):
            ...
            run_stats = self._run_manager.docker.get_container_stats(
                run_state.container_id
            )
            ...
            run_state = run_state._replace(
                disk_utilization=self._run_manager.disk_utilization[bundle_uuid]
                ['disk_utilization']
            )

            if (
                run_state.resources['request_time'] and
            ...
            if (
                run_state.resources['request_disk'] and
                run_state.disk_utilization > run_state.resources['request_disk']
            ):
                kill_messages.append(
        ...
        def check_disk_utilization():
            ...
            while running:
                ...
                try:
                    ...
                    running = self._run_manager.disk_utilization[bundle_uuid
                                                                ]['running']
                except Exception:
                    traceback.print_exc()
                end_time = time.time()
        ...
        if run_state.info['finished']:
            logger.debug(
                ...
                bundle_uuid, run_state.info['exitcode'],
                run_state.info['failure_message']
            )
            self._run_manager.disk_utilization[bundle_uuid]['running'] = False
            self._run_manager.disk_utilization.remove(bundle_uuid)
    ...
    def _transition_from_CLEANING_UP(self, run_state):
        ...
        if run_state.container_id is not None:
            while True:
                try:
                    finished, _, _ = self._run_manager.docker.check_finished(
                        run_state.container_id
                    )
                    if finished:
                        self._run_manager.docker.delete_container(
                            run_state.container_id
                        )
                        break
                except DockerException:
    ...
    def _transition_from_UPLOADING_RESULTS(self, run_state):
        ...
        def upload_results():
            try:
                # Upload results
                logger.debug(
                    'Uploading results for run with UUID %s', bundle_uuid
                )
    ...
    def finalize_run(self, run_state):
        ...
        return run_state._replace(
            stage=LocalRunStage.FINALIZING,
            info=run_state.info,
    ...
    def _transition_from_FINALIZING(self, run_state):
        ...
        if run_state.info['finalized']:
            remove_path(run_state.bundle_path)
            return run_state._replace(
                stage=LocalRunStage.FINISHED, run_status='Finished'
            )
        else:
            return run_state
