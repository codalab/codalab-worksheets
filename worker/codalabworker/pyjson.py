...
class PyJSONEncoder(json.JSONEncoder):
    ...
    def encode_key(self, key):
        ...
        """
        if not (
            isinstance(key, tuple) or isinstance(key, str) or
            isinstance(key, unicode)
        ):
            raise Exception(
                'PyJSON can only encode dicts with str, unicode or tuple keys'
            )
        if isinstance(key, tuple):
            if not all(
                isinstance(tuple_el, str) or isinstance(tuple_el, unicode)
                for tuple_el in key
            ):
                raise Exception(
            ...
            if any(
                PyJSONEncoder.TUPLE_ELEM_STR in tuple_el for tuple_el in key
            ):
                raise Exception(
                    '%s is reserved for pyjson encoder but found in keys. Can\'t encode this dict'
...
class PyJSONDecoder(json.JSONDecoder):
    ...
    def __init__(self, *args, **kwargs):
        json.JSONDecoder.__init__(
            self, *args, object_hook=self.json_as_python, **kwargs
        )
    ...
    def json_as_python(self, dct):
        if isinstance(dct, dict):
            if '_set_object' in dct:
                return set(
                    self.json_as_python(item) for item in dct['_set_object']
                )
            elif '_namedtuple_name' in dct:
                ntc = namedtuple(
                    dct['_namedtuple_name'], dct['_namedtuple_fields']
                )
                t = list(
                    self.json_as_python(item)
                    for item in dct['_namedtuple_values']
                )
                return ntc(*t)
            elif '_tuple_object' in dct:
                return tuple(
                    self.json_as_python(item) for item in dct['_tuple_object']
                )
            else:
                return {
                    self.decode_key(k): self.json_as_python(v)
