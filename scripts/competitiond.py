...
class JsonApiClientWithRetry(JsonApiClient):
    ...
    def _make_request(self, *args, **kwargs):
        ...
        while True:
            try:
                return super(JsonApiClientWithRetry, self)._make_request(
                    *args, **kwargs
                )
...
class MimicReplacementSchema(Schema):
    ...
    new = fields.String(
        validate=validate_uuid, required=True, metadata="uuid of bundle to swap in"
    )


class MimicConfigSchema(Schema):
    tag = fields.String(
        missing="competition-predict", metadata="how to tag new prediction bundles"
    )
    metadata = fields.Dict(
...
class ConfigSchema(Schema):
    ...
    max_leaderboard_size = fields.Integer(
        missing=10000,
        metadata="maximum number of bundles you expect to have on the log worksheet",
    )
    ...
    make_predictions_public = fields.Boolean(
        missing=False,
        metadata="whether to make newly-created prediction bundles publicly readable",
    )
...
class AuthHelper(object):
    ...
    def get_access_token(self):
        if (
            not self.grant
            or time.time() > self.expires_at - self.REFRESH_BUFFER_SECONDS
        ):
...
class Competition(object):
    ...
    def _clear_competition_metadata(self, bundle):
        ...
        self.client.update(
            "bundles", {"id": bundle["id"], "metadata": {"description": ""}}
        )
            ...
            [
                {
                    "id": b["id"],
                    "metadata": {
                        "tags": [t for t in b["metadata"]["tags"] if t != tag]
                    },
                }
                for b in bundles
            ],
    ...
    def _fetch_latest_submissions(self):
        ...
        for bundle in reversed(all_submissions):
            ...
            if (
                key not in submissions
                or created > submissions[key]["metadata"]["created"]
            ):
    ...
    def _filter_submissions(
        ...
        for key, bundle in submissions.items():
            ...
            if (
                num_total_submissions[key.owner_id]
            ...
            if (
                num_period_submissions[key.owner_id]
    ...
    def run_prediction(self, submit_bundle):
        ...
        assert (
            predict_bundle is not None
        ), "Unexpected error: couldn't find mimicked bundle in plan"
    ...
    def _fetch_scores(self, eval_bundles):
        ...
        for bundle in eval_bundles.itervalues():
            if bundle["state"] == State.READY:
                ...
                scores[bundle["id"]] = {
                    spec["name"]: None for spec in self.config["score_specs"]
                }
    ...
    def generate_leaderboard(self, num_total_submissions, num_period_submissions):
        ...
        leaderboard.sort(
            key=lambda e: tuple(
                e["scores"][spec["name"]] for spec in self.config["score_specs"]
            ),
            reverse=True,
        )
    ...
    def run_once(self):
        submissions, num_total_submissions, num_period_submissions = (
...
def main():
    ...
    parser.add_argument(
        "-v", "--verbose", action="store_true", help="Output verbose log messages."
    )
