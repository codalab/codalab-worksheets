...
class SlurmWorkerDaemon(Daemon):
            ...
            for uuid in uuids:
                ...
                if uuid not in cooldown_runs:
                    ...
                    run = {
                        ...
                        for field, val in zip(SlurmWorkerDaemon.FIELDS, field_values.split())
                    }
    ...
    def cleanup(self):
        """
        Do the necessary cleanup before exiting
        - Clear all the logs and other caches?
        """
    ...
    @classmethod
    def parse_host(cls, tags):
        for tag in tags:
            matches = re.match(cls.HOSTS_REGEX, tag)
...
def parse_args():
    ...
    parser.add_argument('--default-tag', type=str, help='Tag to use for workers', default=None)
    parser.add_argument(
        '--root-dir',
        type=str,
