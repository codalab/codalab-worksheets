...
CODALAB_ARGUMENTS = [
    ...
    CodalabArg(
        name='worker_manager_aws_default_cpus',
        type=int,
        default=1,
        help='Default number of CPUs for each worker in the AWS Batch Worker Manager',
    ),
    CodalabArg(
        name='worker_manager_aws_default_memory_mb',
        type=int,
        default=2048,
    ...
    CodalabArg(
        name='link_mounts',
        help='Comma-separated list of directories that are mounted on the CodaLab server, allowing their contents to be used in the --link argument.',
    ),
    ### Public workers
...
for worker_manager_type in ['cpu', 'gpu']:
    CODALAB_ARGUMENTS += [
        CodalabArg(
            name='worker_manager_{}_queue'.format(worker_manager_type),
