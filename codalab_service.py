...
ALL_SERVICES = DEFAULT_SERVICES + ['test', 'monitor', 'aws-batch-worker']
...
def should_run_service(args, service):
    ...
    if service == 'local-worker-shared-file-system':
...
CODALAB_ARGUMENTS = [
    ...
    ### AWS Batch Worker
    CodalabArg(name='worker_batch_queue', help='AWS Batch queue to use for the batch worker'),
]
...
class CodalabServiceManager(object):
    ...
    def start_services(self):
        ...
        if self.args.shared_file_system:
            self.bring_up_service('local-worker-shared-file-system')
        else:
            self.bring_up_service('local-worker')
        self.bring_up_service('aws-batch-worker')
