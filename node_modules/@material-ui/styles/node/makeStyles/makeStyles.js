...
function useSynchronousEffect(func, values) {
  const key = _react.default.useRef([]);

  let output; // Store "generation" key. Just returns a new object every time

  const currentKey = _react.default.useMemo(() => ({}), values); // eslint-disable-line react-hooks/exhaustive-deps
...
function makeStyles(stylesOrCreator, options = {}) {
  const {
    ...
    defaultTheme = _noopTheme.default
        ...
        stylesOptions2 = (0, _objectWithoutPropertiesLoose2.default)(options, ["name", "classNamePrefix", "Component", "defaultTheme"]);
  ...
  const useStyles = (props = {}) => {
    ...
    const stylesOptions = (0, _extends2.default)({}, _react.default.useContext(_StylesProvider.StylesContext), stylesOptions2);

    const instance = _react.default.useRef();

    const shouldUpdate = _react.default.useRef();
