...
parser.add_argument(
    "--backup-path", help="directory to backup database", default="monitor.backup"
)
...
def send_email(subject, message):
    ...
    if not recipient or not sender_info:
        print(
            "send_email; subject: %s; message contains %d lines"
            % (subject, len(message))
        )
        return
...
def run_command(args, soft_time_limit=15, hard_time_limit=60, include_output=True):
    ...
    message = (
        ">> %s (exit code %s, time %.2fs [limit: %ds,%ds]; avg %.2fs; max %.2fs)\n%s"
        % (
            " ".join(args),
            exitcode,
            duration,
