...
parser.add_argument(
    ...
    default=os.getenv(
        'CODALAB_HOME', os.path.join(os.getenv('HOME'), '.codalab')
    )
)

# Where to write out information
parser.add_argument(
    '--log-path', help='file to write the log', default='monitor.log'
)
parser.add_argument(
    '--backup-path',
    help='directory to backup database',
    default='monitor.backup'
)

# How often to do things
parser.add_argument(
    '--ping-interval',
    help='ping the server every this many seconds',
    type=int,
    default=30
)
parser.add_argument(
    '--run-interval',
    help='run a job every this many seconds',
    type=int,
    default=5 * 60
)
parser.add_argument(
...
print 'user = {}, password = {}, db = {}, host = {}, port = {}'.format(
    bundles_user, '*' * len(bundles_password), bundles_db, bundles_host,
    bundles_port
)
...
def send_email(subject, message):
    ...
    if not recipient or not sender_info:
        print 'send_email; subject: %s; message contains %d lines' % (
            subject, len(message)
        )
        return
...
def run_command(
    args, soft_time_limit=15, hard_time_limit=60, include_output=True
):
...
def check_disk_space(paths):
    ...
    if total < 1000 * 1024:
        error_logs(
            'low disk space', 'Only %s MB of disk space left on %s!' %
            (total / 1024, ' '.join(paths))
        )
    ...
    try:
        ...
        if run_time():
            upload_uuid = run_command(
                [
                    'cl', 'upload',
                    os.path.join(CODALAB_CLI, 'scripts', 'stress-test.pl')
                ]
            )
            cat_result = run_command(
                ['cl', 'cat', upload_uuid], include_output=False
            )
            ...
            uuid = run_command(
                [
                    'cl', 'run', 'stress-test.pl:' + upload_uuid,
                    'perl stress-test.pl 5 10 10'
                ]
            )
            run_command(['cl', 'wait', uuid], 30,
                        300)  # Running might take a while
