...
class WorkerModel(object):
    ...
    def worker_checkin(
        self, user_id, worker_id, tag, cpus, gpus, memory_bytes, dependencies
    ):
        """
        ...
        with self._engine.begin() as conn:
            ...
            if existing_row:
                conn.execute(
                    cl_worker_dependency.update().where(
                        and_(
    ...
    def get_bundle_worker(self, uuid):
        ...
        with self._engine.begin() as conn:
            ...
            precondition(
                row, 'Trying to find worker for bundle that is not running.'
            )
            worker_row = conn.execute(
                cl_worker.select().where(
                    and_(
                        cl_worker.c.user_id == row.user_id,
    ...
    def allocate_socket(self, user_id, worker_id, conn=None):
        ...
        def do(conn):
            ...
            return conn.execute(cl_worker_socket.insert().values(socket_row)
    ...
    def send_json_message(
        self, socket_id, message, timeout_secs, autoretry=True
    ):
        """
            ...
            with closing(socket.socket(socket.AF_UNIX,
                ...
                try:
                    ...
                    if autoretry:
                        ...
                        success = sock.recv(len(WorkerModel.ACK)
                                           ) == WorkerModel.ACK
                    else:
                        success = True
                except socket.error:
