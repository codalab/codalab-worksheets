...
event = Table(
    ...
    Column(
        "command", String(63), nullable=False
    ),  # The command (gotten in bundle_rpc_server)
...
oauth2_client = Table(
    ...
    Column(
        "redirect_uris", Text, nullable=False
...
oauth2_token = Table(
    ...
    Column("id", Integer, primary_key=True, nullable=False),
    Column(
        "client_id", String(63), ForeignKey(oauth2_client.c.client_id), nullable=False
    ),
    Column("user_id", String(63), ForeignKey(user.c.user_id), nullable=False),
...
worker = Table(
    ...
    Column(
        "socket_id", Integer, nullable=False
    ),  # Socket ID worker listens for messages on.
)
