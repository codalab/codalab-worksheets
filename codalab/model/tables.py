...
bundle_metadata = Table(
    ...
    Column('id', Integer, primary_key=True, nullable=False),
    Column(
        'bundle_uuid', String(63), ForeignKey(bundle.c.uuid), nullable=False
    ),
    Column('metadata_key', String(63), nullable=False),
    Column('metadata_value', Text, nullable=False),
    Index(
        'metadata_kv_index', 'metadata_key', 'metadata_value', mysql_length=63
    ),
    sqlite_autoincrement=True,
)
...
worksheet = Table(
    ...
    Column('title', Unicode(255),
           nullable=True),  # Short human-readable description of the worksheet
...
group_bundle_permission = Table(
    ...
    Column(
        'object_uuid', String(63), ForeignKey(bundle.c.uuid), nullable=False
    ),
...
group_object_permission = Table(
    ...
    Column(
        'object_uuid', String(63), ForeignKey(worksheet.c.uuid), nullable=False
    ),
    # Permissions encoded as integer (see below)
    Column('permission', Integer, nullable=False),
    sqlite_autoincrement=True,
...
user = Table(
    ...
    Column('email', String(254), nullable=False,
           unique=True),  # Length of 254 to be compliant with RFC3696/5321
    Column(
    ...
    Column('time_used', Float,
           nullable=False),  # Number of seconds already used
...
oauth2_token = Table(
    ...
    Column(
        'client_id',
        String(63),
        ForeignKey(oauth2_client.c.client_id),
        nullable=False
    ),
    Column('user_id', String(63), ForeignKey(user.c.user_id), nullable=False),
...
chat = Table(
    ...
    Column('time', DateTime,
           nullable=False),  # When did the user send this query?
...
worker = Table(
    ...
    Column('tag', Text, nullable=True
          ),  # Tag that allows for scheduling runs on specific workers.
    Column('cpus', Integer, nullable=False),  # Number of CPUs on worker.
    Column('gpus', Integer, nullable=False),  # Number of GPUs on worker.
    Column('memory_bytes', BigInteger,
           nullable=False),  # Total memory of worker.
    ...
    Column('socket_id', Integer,
           nullable=False),  # Socket ID worker listens for messages on.
)
