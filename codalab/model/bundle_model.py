...
class BundleModel(object):
    ...
    def get_bundle(self, uuid):
        ...
        if not bundles:
            ...
            raise IntegrityError(
                'Found multiple bundles with uuid %s' % (uuid, )
            )
        return bundles[0]
    ...
    def search_bundles(self, user_id, keywords):
        ...
        sum_key = [None]
        aux_fields = [
        ]  # Fields (e.g., sorting) that we need to include in the query
        ...
        for keyword in keywords:
                ...
                clause = cl_bundle.c.uuid.in_(
                    select([cl_group_bundle_permission.c.object_uuid]).where(
                        and_(
                            ...
                            group_uuid, cl_group_bundle_permission.c.permission
                            >= GROUP_OBJECT_PERMISSION_READ
                        )
                    )
                )
            # Special fields
                ...
                condition = make_condition(
                    key, cl_bundle_dependency.c.parent_uuid, value
                )
                ...
                if condition is None:  # top-level
                    clause = and_(
                        cl_bundle_dependency.c.child_uuid ==
                        cl_bundle.c.uuid,  # Join constraint
                ...
                condition = make_condition(
                    key, cl_worksheet_item.c.worksheet_uuid, value
                )
                if condition is None:  # top-level
                    ...
                    clause = cl_bundle.c.uuid.in_(
                        alias(
                            select([cl_worksheet_item.c.bundle_uuid]
                                  ).where(condition)
                        )
                    )
            ...
            # Otherwise, assume metadata.
                ...
                condition = make_condition(
                    key, cl_bundle_metadata.c.metadata_value, value
                )
                if condition is None:  # top-level
                    clause = and_(
                        ...
                        or_(  # Join constraint (group)
                            ...
                            cl_group_bundle_permission.c.group_uuid.in_(
                                alias(
                                    select([
                                        cl_user_group.c.group_uuid
                                    ]).where(cl_user_group.c.user_id ==
                                             user_id)))  # Private group
                        ),
                        cl_group_bundle_permission.c.permission >=
        ...
        if sum_key[0] is not None:
            ...
            query = alias(
                select([cl_bundle.c.uuid,
                        sum_key[0].label('num')]).distinct().where(clause)
            )
            # Sum the numbers
            query = select([func.sum(query.c.num)])
        else:
            query = select([cl_bundle.c.uuid] + aux_fields).distinct(
            ).where(clause).offset(offset).limit(limit)
        ...
        if 'uuid' in conditions:
            ...
            if conditions['name']:
                clause = and_(
                    cl_bundle_metadata.c.metadata_key == 'name',
                    self.make_clause(
                        cl_bundle_metadata.c.metadata_value, conditions['name']
                    )
                )
            else:
                clause = true()
            ...
            if conditions['worksheet_uuid']:
                ...
                clause = and_(
                    ...
                    self.make_clause(
                        cl_worksheet_item.c.worksheet_uuid,
                        conditions['worksheet_uuid']
                    )
                )
                clause = and_(
                    clause, cl_worksheet_item.c.bundle_uuid is not None
                )
                join = cl_worksheet_item.outerjoin(
                    cl_bundle_metadata, cl_worksheet_item.c.bundle_uuid ==
                    cl_bundle_metadata.c.bundle_uuid
                )
                query = select(
                    [cl_worksheet_item.c.bundle_uuid, cl_worksheet_item.c.id]
                ).select_from(join).distinct().where(clause)
                query = query.order_by(cl_worksheet_item.c.id.desc()
                                      ).limit(max_results)
            else:
                ...
                clause = and_(
                    clause, cl_bundle.c.uuid == cl_bundle_metadata.c.bundle_uuid
                )  # Join
                query = select([cl_bundle.c.uuid]).where(clause)
    ...
    def batch_get_bundles(self, **kwargs):
        ...
        with self.engine.begin() as connection:
            bundle_rows = connection.execute(cl_bundle.select().where(clause)
                                            ).fetchall()
            if not bundle_rows:
                return []
        ...
        for metadata_row in metadata_rows:
            ...
            bundle_values[metadata_row.bundle_uuid]['metadata'
                                                   ].append(metadata_row)

        # Construct and validate all of the retrieved bundles.
        sorted_values = sorted(
            bundle_values.itervalues(), key=lambda r: r['id']
        )
        bundles = [
        ...
        with self.engine.begin() as connection:
            ...
            row = connection.execute(
                cl_bundle.select().where(
                    cl_bundle.c.id == bundle.id and (
                        cl_bundle.c.state == State.RUNNING or
                        cl_bundle.c.state == State.PREPARING
                    )
                )
            ).fetchone()
            if not row:
            ...
            connection.execute(
                cl_worker_run.delete().where(
                    cl_worker_run.c.run_uuid == bundle.uuid
                )
            )
    ...
    def restage_bundle(self, bundle):
        ...
        with self.engine.begin() as connection:
            ...
            if not row:
                raise IntegrityError(
                    'Missing bundle with UUID %s' % bundle.uuid
                )
    ...
    def bundle_checkin(
        self, bundle, bundle_update, user_id, worker_id, hostname
    ):
        '''
        ...
        with self.engine.begin() as connection:
            ...
            if state == State.FINALIZING:
                return self.finalize_bundle(
                    bundle, user_id, bundle_update['info']['exitcode'],
                    bundle_update['info']['failure_message'], connection
                )
            elif state in [State.PREPARING, State.RUNNING]:
                return self.resume_bundle(
                    bundle, bundle_update, row, user_id, worker_id, hostname,
                    connection
                )
            else:
    ...
    def resume_bundle(
        self, bundle, bundle_update, row, user_id, worker_id, hostname,
        connection
    ):
        '''
    ...
    def finalize_bundle(
        self, bundle, user_id, exitcode, failure_message, connection
    ):
        """
        ...
        if user_id == self.root_user_id:
            self.increment_user_time_used(
                bundle.owner_id, getattr(bundle.metadata, 'time', 0)
            )
    ...
    def save_bundle(self, bundle):
        ...
        with self.engine.begin() as connection:
            ...
            self.do_multirow_insert(
                connection, cl_bundle_dependency, dependency_values
            )
            self.do_multirow_insert(
                connection, cl_bundle_metadata, metadata_values
            )
    ...
    def update_bundle(self, bundle, update, connection=None):
        ...
        def do_update(connection):
            ...
            if metadata_update:
                connection.execute(
                    cl_bundle_metadata.delete().where(metadata_clause)
                )
                self.do_multirow_insert(
                    connection, cl_bundle_metadata, metadata_values
                )
    ...
    def get_bundle_states(self, uuids):
        ...
        with self.engine.begin() as connection:
            rows = connection.execute(
                select([cl_bundle.c.uuid,
                        cl_bundle.c.state]).where(cl_bundle.c.uuid.in_(uuids))
            ).fetchall()
    ...
    def delete_bundles(self, uuids):
        ...
        with self.engine.begin() as connection:
            ...
            connection.execute(
                cl_bundle_metadata.delete().where(
                    cl_bundle_metadata.c.bundle_uuid.in_(uuids)
                )
            )
            connection.execute(
                cl_bundle_dependency.delete().where(
                    cl_bundle_dependency.c.child_uuid.in_(uuids)
                )
            )
            connection.execute(
                cl_bundle.delete().where(cl_bundle.c.uuid.in_(uuids))
            )

    def remove_data_hash_references(self, uuids):
        with self.engine.begin() as connection:
            connection.execute(
                cl_bundle.update().where(cl_bundle.c.uuid.in_(uuids)).values(
                    {
                        'data_hash': None
                    }
                )
    ...
    def get_worksheet(self, uuid, fetch_items):
        ...
        :rtype: Worksheet
        """
        worksheets = self.batch_get_worksheets(
            fetch_items=fetch_items, uuid=uuid
        )
        if not worksheets:
            raise NotFoundError(
                'Could not find worksheet with uuid %s' % (uuid, )
            )
        elif len(worksheets) > 1:
            raise IntegrityError(
                'Found multiple workseets with uuid %s' % (uuid, )
            )
        return worksheets[0]
        ...
        with self.engine.begin() as connection:
            ...
            if fetch_items:
                item_rows = connection.execute(
                    cl_worksheet_item.select().where(
        ...
        if fetch_items:
            ...
            for item_row in sorted(item_rows, key=item_sort_key):
                if item_row.worksheet_uuid not in worksheet_values:
                    raise IntegrityError(
                        'Got item %s without worksheet' % (item_row, )
                    )
                item_row = dict(item_row)
                item_row['value'] = self.decode_str(item_row['value'])
                worksheet_values[item_row['worksheet_uuid']]['items'
                                                            ].append(item_row)
    ...
    def search_worksheets(self, user_id, keywords):
        ...
        for keyword in keywords:
            ...
            # Bundle fields
                ...
                condition = make_condition(
                    cl_worksheet_item.c.bundle_uuid, value
                )
                if condition is None:  # top-level
                    ...
                    clause = cl_worksheet.c.uuid.in_(
                        alias(
                            select([cl_worksheet_item.c.worksheet_uuid]
                                  ).where(condition)
                        )
                    )
            elif key == 'worksheet':  # contains worksheet?
                condition = make_condition(
                    cl_worksheet_item.c.subworksheet_uuid, value
                )
                if condition is None:  # top-level
                    clause = cl_worksheet_item.c.worksheet_uuid == cl_worksheet.c.uuid  # Join constraint
                else:
                ...
                if condition is None:  # top-level
                    ...
                    clause = cl_worksheet.c.uuid.in_(
                        alias(
                            select([cl_worksheet_tag.c.worksheet_uuid]
                                  ).where(condition)
                        )
                    )
        ...
        cols_to_select = [
            cl_worksheet.c.id, cl_worksheet.c.uuid, cl_worksheet.c.name,
            cl_worksheet.c.title, cl_worksheet.c.frozen, cl_worksheet.c.owner_id
        ]
        query = select(cols_to_select
                      ).distinct().where(clause).offset(offset).limit(limit)
    ...
    def new_worksheet(self, worksheet):
        ...
        """
        message = 'save_worksheet called with non-empty worksheet: %s' % (
            worksheet,
        )
        ...
        with self.engine.begin() as connection:
            result = connection.execute(
                cl_worksheet.insert().values(worksheet_value)
            )
        ...
        with self.engine.begin() as connection:
            ...
            for old_item in old_items:
                new_item = {
                    ...
                    'value':
                        '',  # TODO: replace with None once we change tables.py
                    'sort_key':
                        old_item.
                        sort_key,  # Can't really do after, so use the same value.
                }
                new_items.append(new_item)
                connection.execute(cl_worksheet_item.insert().values(new_item))
            # sqlite doesn't support batch insertion

    def update_worksheet_items(
        self, worksheet_uuid, last_item_id, length, new_items
    ):
        """
        ...
        new_item_values = [
            ...
            } for (i, (bundle_uuid, subworksheet_uuid, value,
                       type)) in enumerate(new_items)
        ]
        with self.engine.begin() as connection:
            result = connection.execute(
            ...
            if result.rowcount < length:
                raise UsageError(
                    'Worksheet %s was updated concurrently!' %
                    (worksheet_uuid, )
                )
            self.do_multirow_insert(
                connection, cl_worksheet_item, new_item_values
            )
    ...
    def update_worksheet_metadata(self, worksheet, info):
        ...
        with self.engine.begin() as connection:
            if 'tags' in info:
                ...
                new_tag_values = [
                    {
                        'worksheet_uuid': worksheet.uuid,
                        'tag': tag
                    } for tag in info['tags']
                ]
                self.do_multirow_insert(
                    connection, cl_worksheet_tag, new_tag_values
                )
                del info['tags']
            if len(info) > 0:
                connection.execute(
                    cl_worksheet.update().where(
        ...
        with self.engine.begin() as connection:
            connection.execute(
                cl_group_worksheet_permission.delete().where(
                    cl_group_worksheet_permission.c.object_uuid ==
                    worksheet_uuid
                )
            )
            connection.execute(
    ...
    def batch_get_groups(self, **kwargs):
        ...
        with self.engine.begin() as connection:
            rows = connection.execute(cl_group.select().where(clause)
                                     ).fetchall()
            if not rows:
                return []
    ...
    def batch_get_all_groups(
        self, spec_filters, group_filters, user_group_filters
    ):
        """
        ...
        fetch_cols2 = fetch_cols + [
            cl_user_group.c.user_id, cl_user_group.c.is_admin
        ]
        ...
        if spec_filters:
            ...
            q2 = select(fetch_cols2).where(spec_clause).where(
                cl_group.c.uuid == cl_user_group.c.group_uuid
            )
        if True:
            if q0 is None:
        ...
        if user_group_filters:
            user_group_clause = self.make_kwargs_clause(
                cl_user_group, user_group_filters
            )
            if q2 is None:
                q2 = select(fetch_cols2).where(
                    cl_group.c.uuid == cl_user_group.c.group_uuid
                )
    ...
    def delete_group(self, uuid):
        ...
        with self.engine.begin() as connection:
            connection.execute(
                cl_group_bundle_permission.delete()
                .where(cl_group_bundle_permission.c.group_uuid == uuid)
            )
            connection.execute(
                cl_group_worksheet_permission.delete()
    ...
    def update_user_in_group(self, user_id, group_uuid, is_admin):
        ...
        with self.engine.begin() as connection:
            connection.execute(
                ...
                .where(cl_user_group.c.user_id == user_id)
                .where(cl_user_group.c.group_uuid == group_uuid).values(
                    {
                        'is_admin': is_admin
                    }
                )
    ...
    def batch_get_user_in_group(self, **kwargs):
        ...
        with self.engine.begin() as connection:
            rows = connection.execute(cl_user_group.select().where(clause)
                                     ).fetchall()
            if not rows:
                return []
    ...
    def _get_user_groups(self, user_id):
        groups = [
            self.public_group_uuid
        ]  # Everyone is in the public group implicitly.
        if user_id is not None:
            groups += [
                row['group_uuid']
                for row in self.batch_get_user_in_group(user_id=user_id)
            ]
        return groups

    def set_group_permission(
        self, table, group_uuid, object_uuid, new_permission
    ):
        """
        ...
        with self.engine.begin() as connection:
            ...
            if new_permission > 0:
                if old_permission > 0:
                    ...
                    connection.execute(
                        table.update().where(table.c.group_uuid == group_uuid)
                        .where(table.c.object_uuid == object_uuid).values(
                            {
                ...
                if old_permission > 0:
                    # Delete permission
                    connection.execute(
                        table.delete().where(table.c.group_uuid == group_uuid)
                        .where(table.c.object_uuid == object_uuid)
                    )

    def set_group_bundle_permission(
        self, group_uuid, bundle_uuid, new_permission
    ):
        return self.set_group_permission(
            cl_group_bundle_permission, group_uuid, bundle_uuid, new_permission
        )

    def set_group_worksheet_permission(
        self, group_uuid, worksheet_uuid, new_permission
    ):
        return self.set_group_permission(
            cl_group_worksheet_permission, group_uuid, worksheet_uuid,
            new_permission
        )
    ...
    def batch_get_group_permissions(self, table, user_id, object_uuids):
        ...
        with self.engine.begin() as connection:
            ...
            rows = connection.execute(
                ...
                .where(group_restrict).where(
                    table.c.object_uuid.in_(object_uuids)
                ).order_by(cl_group.c.name)
            ).fetchall()
            result = collections.defaultdict(
                list
            )  # object_uuid => list of rows
            for row in rows:
    ...
    def batch_get_group_bundle_permissions(self, user_id, bundle_uuids):
        return self.batch_get_group_permissions(
            cl_group_bundle_permission, user_id, bundle_uuids
        )
    ...
    def get_user_permissions(self, table, user_id, object_uuids, owner_ids):
        ...
        object_permissions = dict(
            (object_uuid, GROUP_OBJECT_PERMISSION_NONE)
        ...
        if len(remaining_object_uuids) > 0:
            result = self.batch_get_group_permissions(
                table, user_id, remaining_object_uuids
            )
    ...
    def get_user_worksheet_permissions(
        self, user_id, worksheet_uuids, owner_ids
    ):
        return self.get_user_permissions(
    ...
    def get_events_log_info(self, query_info, offset, limit):
        ...
        if field_name is not None:
            if field_name == 'user':
                ...
                raise UsageError(
                    "Invalid field: '%s', expected user|command|uuid|date" %
                    field_name
                )
    ...
    def update_events_log(
        self, user_id, user_name, command, args, start_time=None, uuid=None
    ):
    ...
    def date_handler(self, obj):
        ...
        """
        return obj.isoformat() if isinstance(
    ...
    def get_chat_log_info(self, query_info):
        ...
        with self.engine.begin() as connection:
            query = select(
                [
                    cl_chat.c.time, cl_chat.c.sender_user_id,
                    cl_chat.c.recipient_user_id, cl_chat.c.message
                ]
            )
            clause = []
            ...
            if user_id1 == self.root_user_id:
                ...
                clause.append(
                    cl_chat.c.recipient_user_id == self.system_user_id
                )
            clause = or_(*clause)
    ...
    def find_user(self, user_spec, check_active=True):
        user = self.get_user(
            user_id=user_spec, username=user_spec, check_active=check_active
        )
        if user is None:
    ...
    def get_users(self, user_ids=None, usernames=None, check_active=True):
        ...
        if usernames is not None:
            clauses.append(
                or_(
                    cl_user.c.user_name.in_(usernames),
                    cl_user.c.email.in_(usernames)
                )
            )

        with self.engine.begin() as connection:
            rows = connection.execute(select([cl_user]).where(and_(*clauses))
                                     ).fetchall()
                ...
                cl_user.insert().values(
                    {
                        ...
                        "password":
                            User.encode_password(
                                password, crypt_util.get_random_string()
                            ),
                        "time_quota":
                            self.default_user_info['time_quota'],
                        "time_used":
                            0,
                        "disk_quota":
                            self.default_user_info['disk_quota'],
                        "disk_used":
                            0,
                        "affiliation":
                            affiliation,
    ...
    def delete_user(self, user_id=None):
        ...
        with self.engine.begin() as connection:
            ...
            # User verification
            connection.execute(
                cl_user_verification.delete().where(
                    cl_user_verification.c.user_id == user_id
                )
            )
            connection.execute(
                cl_user_reset_code.delete().where(
            ...
            # OAuth2
            connection.execute(
                oauth2_auth_code.delete().where(
                    oauth2_auth_code.c.user_id == user_id
                )
            )
            connection.execute(
                oauth2_token.delete().where(oauth2_token.c.user_id == user_id)
            )
            connection.execute(
                oauth2_client.delete().where(
            ...
            # Event
            connection.execute(
                cl_event.delete().where(cl_event.c.user_id == user_id)
            )
            ...
            # Delete User
            connection.execute(
                cl_user.delete().where(cl_user.c.user_id == user_id)
            )
    ...
    def get_verification_key(self, user_id):
        ...
        with self.engine.begin() as connection:
            verify_row = connection.execute(
                cl_user_verification.select().where(
                    cl_user_verification.c.user_id == user_id
                ).limit(1)
            ).fetchone()
            ...
            if verify_row is None:
                ...
                # Update date sent
                connection.execute(
                    cl_user_verification.update().where(
                        cl_user_verification.c.user_id == user_id
                    ).values({
    ...
    def get_reset_code_user_id(self, code, delete=False):
        ...
        with self.engine.begin() as connection:
            reset_code_row = connection.execute(
                cl_user_reset_code.select().where(
    ...
    def get_user_info(self, user_id, fetch_extra=False):
        ...
        with self.engine.begin() as connection:
            rows = connection.execute(
                select([cl_user]).where(cl_user.c.user_id == user_id)
            )
            user_info = None
            for row in rows:
            ...
            if fetch_extra:
                if 'date_joined' in user_info and user_info['date_joined'
                                                           ] is not None:
                    user_info['date_joined'] = user_info['date_joined'
                                                        ].strftime('%Y-%m-%d')
                if 'last_login' in user_info and user_info['last_login'
                                                          ] is not None:
                    user_info['last_login'] = user_info['last_login'
                                                       ].strftime('%Y-%m-%d')
                user_info['is_root_user'] = True if user_info[
                    'user_id'
                ] == self.root_user_id else False
    ...
    def update_user_info(self, user_info):
        ...
        with self.engine.begin() as connection:
            connection.execute(
                cl_user.update().where(
                    cl_user.c.user_id == user_info['user_id']
                ).values(user_info)
            )
    ...
    def _get_disk_used(self, user_id):
        return self.search_bundles(
            user_id, ['size=.sum', 'owner_id=' + user_id, 'data_hash=%']
        )['result'] or 0
    ...
    def get_oauth2_client(self, client_id):
        with self.engine.begin() as connection:
            row = connection.execute(
                select([oauth2_client]
                      ).where(oauth2_client.c.client_id == client_id).limit(1)
            ).fetchone()
    ...
    def save_oauth2_client(self, client):
        with self.engine.begin() as connection:
            result = connection.execute(
                oauth2_client.insert().values(client.columns)
            )
    ...
    def get_oauth2_token(self, access_token=None, refresh_token=None):
        ...
        with self.engine.begin() as connection:
            row = connection.execute(
                select([oauth2_token]).where(clause).limit(1)
            ).fetchone()
    ...
    def save_oauth2_token(self, token):
        with self.engine.begin() as connection:
            result = connection.execute(
                oauth2_token.insert().values(token.columns)
            )
    ...
    def delete_oauth2_token(self, token_id):
        with self.engine.begin() as connection:
            connection.execute(
                oauth2_auth_code.delete().where(oauth2_token.c.id == token_id)
            )

    def get_oauth2_auth_code(self, client_id, code):
        with self.engine.begin() as connection:
            row = connection.execute(
                select([oauth2_auth_code]).where(
                    and_(
                        oauth2_auth_code.c.client_id == client_id,
                        oauth2_auth_code.c.code == code
                    )
                ).limit(1)
            ).fetchone()
    ...
    def save_oauth2_auth_code(self, grant):
        with self.engine.begin() as connection:
            result = connection.execute(
                oauth2_auth_code.insert().values(grant.columns)
            )
