...
class BundleModel(object):
        ...
        for keyword in keywords:
            ...
            # Special fields
                ...
                if condition is None:  # top-level
                    ...
                    clause = cl_bundle.c.uuid.in_(
                        alias(
                            select([cl_bundle_dependency.c.child_uuid]).where(condition)
                        )
                    )
                ...
                if condition is None:  # top-level
                    ...
                    clause = cl_bundle.c.uuid.in_(
                        alias(
                            select([cl_worksheet_item.c.bundle_uuid]).where(condition)
                        )
                    )
                    ...
                    cl_bundle.c.uuid.in_(
                        alias(
                            select([cl_bundle_metadata.c.bundle_uuid]).where(
                                and_(
                                    cl_bundle_metadata.c.metadata_key == "name",
                                    cl_bundle_metadata.c.metadata_value.like(
                                        "%" + value + "%"
                                    ),
                                )
                            )
        ...
        with self.engine.begin() as connection:
            ...
            row = connection.execute(
                cl_bundle.select().where(
                    ...
                    and (
                        cl_bundle.c.state == State.RUNNING
                        or cl_bundle.c.state == State.PREPARING
                    )
                )
            ).fetchone()
            if not row:
    ...
    def start_bundle(self, bundle, user_id, worker_id, hostname, start_time):
        ...
        with self.engine.begin() as connection:
            ...
            if (
                not run_row
                or run_row.user_id != user_id
                or run_row.worker_id != worker_id
            ):
                return False
    ...
    def resume_bundle(
        self, bundle, bundle_update, row, user_id, worker_id, hostname, connection
    ):
        """
        ...
        self.update_bundle(
            bundle,
            {"state": bundle_update["state"], "metadata": metadata_update},
            connection,
        )
    ...
    def get_bundle_states(self, uuids):
        ...
        with self.engine.begin() as connection:
            rows = connection.execute(
                select([cl_bundle.c.uuid, cl_bundle.c.state]).where(
    ...
    def remove_data_hash_references(self, uuids):
        with self.engine.begin() as connection:
            connection.execute(
                cl_bundle.update()
                .where(cl_bundle.c.uuid.in_(uuids))
                .values({"data_hash": None})
            )
    ...
    def search_worksheets(self, user_id, keywords):
        ...
        for keyword in keywords:
            ...
            if key == ".offset":
                ...
                key == ".shared"
            ):  # shared with any group I am in with read or all permission?
            ...
            # Bundle fields
                ...
                if condition is None:  # top-level
                    ...
                    clause = cl_worksheet.c.uuid.in_(
                        alias(
                            select([cl_worksheet_tag.c.worksheet_uuid]).where(condition)
                        )
                    )
        ...
        query = (
            select(cols_to_select).distinct().where(clause).offset(offset).limit(limit)
        )
    ...
    def update_worksheet_items(self, worksheet_uuid, last_item_id, length, new_items):
        ...
        new_item_values = [
            ...
            for (i, (bundle_uuid, subworksheet_uuid, value, type)) in enumerate(
                new_items
            )
        ]
        with self.engine.begin() as connection:
            ...
            if result.rowcount < length:
                raise UsageError(
                    "Worksheet %s was updated concurrently!" % (worksheet_uuid,)
                )
    ...
    def update_worksheet_metadata(self, worksheet, info):
        ...
        with self.engine.begin() as connection:
            ...
            if len(info) > 0:
                connection.execute(
                    cl_worksheet.update()
                    .where(cl_worksheet.c.uuid == worksheet.uuid)
                    .values(info)
                )
    ...
    def delete_worksheet(self, worksheet_uuid):
        ...
        with self.engine.begin() as connection:
            ...
            connection.execute(
                cl_worksheet.delete().where(cl_worksheet.c.uuid == worksheet_uuid)
            )
    ...
    def delete_group(self, uuid):
        ...
        with self.engine.begin() as connection:
            ...
            connection.execute(
                cl_user_group.delete().where(cl_user_group.c.group_uuid == uuid)
            )
    ...
    def get_group_permissions(self, table, user_id, object_uuid):
        ...
        """
        return self.batch_get_group_permissions(table, user_id, [object_uuid])[
            object_uuid
        ]
    ...
    def get_users(self, user_ids=None, usernames=None, check_active=True):
        ...
        if usernames is not None:
            clauses.append(
                or_(cl_user.c.user_name.in_(usernames), cl_user.c.email.in_(usernames))
            )

        with self.engine.begin() as connection:
            rows = connection.execute(
                select([cl_user]).where(and_(*clauses))
            ).fetchall()
    ...
    def delete_user(self, user_id=None):
        ...
        with self.engine.begin() as connection:
            ...
            connection.execute(
                oauth2_client.delete().where(oauth2_client.c.user_id == user_id)
            )

            # Workers
            connection.execute(
                cl_worker_run.delete().where(cl_worker_run.c.user_id == user_id)
            )

            # User Groups
            connection.execute(
                cl_user_group.delete().where(cl_user_group.c.user_id == user_id)
            )
            ...
            connection.execute(
                cl_chat.delete().where(
                    cl_chat.c.sender_user_id == user_id
                    or cl_chat.c.recipient_user_id == user_id
                )
            )
    ...
    def verify_user(self, key):
        ...
        with self.engine.begin() as connection:
            verify_row = connection.execute(
                cl_user_verification.select()
                .where(cl_user_verification.c.key == key)
                .limit(1)
            ).fetchone()
    ...
    def get_reset_code_user_id(self, code, delete=False):
        ...
        with self.engine.begin() as connection:
            reset_code_row = connection.execute(
                cl_user_reset_code.select()
                .where(cl_user_reset_code.c.code == code)
                .limit(1)
            ).fetchone()
            ...
            connection.execute(
                cl_user_reset_code.delete().where(cl_user_reset_code.c.code == code)
            )
        ...
        with self.engine.begin() as connection:
            ...
            if fetch_extra:
                if "date_joined" in user_info and user_info["date_joined"] is not None:
                    user_info["date_joined"] = user_info["date_joined"].strftime(
                        "%Y-%m-%d"
                    )
                if "last_login" in user_info and user_info["last_login"] is not None:
                    user_info["last_login"] = user_info["last_login"].strftime(
                        "%Y-%m-%d"
                    )
    ...
    def update_user_info(self, user_info):
        ...
        with self.engine.begin() as connection:
            connection.execute(
                cl_user.update()
                .where(cl_user.c.user_id == user_info["user_id"])
                .values(user_info)
            )
    ...
    def update_user_last_login(self, user_id):
        ...
        self.update_user_info(
            {"user_id": user_id, "last_login": datetime.datetime.utcnow()}
        )
    ...
    def get_oauth2_client(self, client_id):
        with self.engine.begin() as connection:
            row = connection.execute(
                ...
                .where(oauth2_client.c.client_id == client_id)
                .limit(1)
            ).fetchone()
