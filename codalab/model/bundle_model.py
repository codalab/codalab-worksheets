...
class BundleModel(object):
    ...
    def transition_bundle_starting(self, bundle, user_id, worker_id):
        ...
        with self.engine.begin() as connection:
            ...
            logger.debug("Yibo: cl_worker.c.is_terminating is " + str(cl_worker.c.is_terminating))
            logger.debug(
                "Yibo: cl_worker.c.is_terminating is  of " + str(type(cl_worker.c.is_terminating))
            )
            row = connection.execute(
                cl_worker.select().where(
