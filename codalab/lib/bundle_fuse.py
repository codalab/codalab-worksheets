...
if fuse_is_available:
    ...
    class ByteRangeReader(object):
        ...
        def __init__(
            self, client, bundle_uuid, timeout=60, max_num_chunks=100, chunk_size=None
        ):
        ...
        def _fetch_chunk(self, path, chunk_id):
            ...
            if len(arr) == self.chunk_size:  # only cache if fetched full chunk
                if (
                    len(self.cache) >= self.max_num_chunks
    ...
    class BundleFuse(Operations):
        ...
        def __init__(self, client, target, verbose=False):
            ...
            self.bundle_metadata = self.client.fetch("bundles", self.bundle_uuid)[
                "metadata"
            ]
        ...
        def read(self, path, length, offset, fh):
            ...
            self.verbose_print(
                "read path={}, length={}, offset={}".format(path, length, offset)
            )
            return result
