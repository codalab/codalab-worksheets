...
class MultiDiskBundleStore(
            ...
            if disk is None:
                ...
                disk = max(
                    ...
                    key=lambda x: self.get_node_avail(
                        os.path.join(
                            self.partitions, x, MultiDiskBundleStore.DATA_SUBDIRECTORY
                        )
                    ),
                )
    ...
    def ls_partitions(self):
        ...
        print(
            "%d %s"
            % (len(partitions), "partition" if len(partitions) == 1 else "partitions")
        )
        for d in partitions:
            ...
            bundles = reduce(
                lambda x, y: x + y,
                path_util.ls(
                    os.path.join(partition_path, MultiDiskBundleStore.DATA_SUBDIRECTORY)
                ),
            )
            print(
                "- %-016s\n\tmountpoint: %s\n\t%d %s"
