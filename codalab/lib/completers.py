    ...
    def __call__(self, prefix, **kwargs):
        ...
        if spec_util.UUID_PREFIX_REGEX.match(worksheet_spec):
            return (
                ...
                for w in worksheets if w['uuid'].startswith(worksheet_spec)
            )
        else:
...
class BundlesCompleter(CodaLabCompleter):
    ...
    def __call__(
        self, prefix, action=None, parsed_args=None, worksheet_uuid=None
    ):
        ...
        if spec_util.UUID_PREFIX_REGEX.match(prefix):
            ...
            return (
                b['metadata']['name'] for b in bundle_infos
...
class AddressesCompleter(CodaLabCompleter):
    ...
    def __call__(self, prefix, action=None, parsed_args=None):
        return (
            a for a in self.cli.manager.config.get('aliases', {})
...
class GroupsCompleter(CodaLabCompleter):
    ...
    def __call__(self, prefix, action=None, parsed_args=None):
        ...
        if spec_util.UUID_PREFIX_REGEX.match(prefix):
            return (
                short_uuid(g['uuid'])
                for g in group_dicts if g['uuid'].startswith(prefix)
            )
        else:
            return (
                g['name'] for g in group_dicts if g['name'].startswith(prefix)
            )
    ...
    class _UnionCompleter(CodaLabCompleter):
        def __call__(self, *args, **kwargs):
            initialized_completers = [
                initialize_completer(completer, self.cli)
            ...
            if info['type'] == 'directory':
                ...
                return (
                    suggestion_format.format(
                        os.path.join(dir_target[1], child_name)
