...
from codalab.lib.cli_util import (
    nested_dict_get, parse_key_target, parse_target_spec, desugar_command,
...
GLOBAL_SPEC_FORMAT = "[%s%s]%s" % (
    ADDRESS_SPEC_FORMAT, INSTANCE_SEPARATOR, BASIC_SPEC_FORMAT
)
WORKSHEET_SPEC_FORMAT = GLOBAL_SPEC_FORMAT

BUNDLE_SPEC_FORMAT = '[%s%s]%s' % (
    WORKSHEET_SPEC_FORMAT, WORKSHEET_SEPARATOR, BASIC_BUNDLE_SPEC_FORMAT
)

TARGET_SPEC_FORMAT = '%s[%s<subpath within bundle>]' % (
    BUNDLE_SPEC_FORMAT, os.sep
)
...
class CodaLabArgumentParser(argparse.ArgumentParser):
    ...
    def error(self, message):
        ...
        if len(
            sys.argv
        ) == 1 or (len(sys.argv) == 2 and sys.argv[1] in ['--help', '-h']):
    ...
    def help_text(cls, verbose):
        ...
        def command_help_text(command):
            ...
            if verbose:
                return '%s%s:\n%s\n%s' % (
                    ' ' * indent, name, '\n'.join(
                        (' ' * (indent * 2)) + line for line in command_obj.help
...
class BundleCLI(object):
    def __init__(
        self, manager, headless=False, stdout=sys.stdout, stderr=sys.stderr
    ):
    ...
    def target_specs_to_bundle_uuids(
        ...
        return [
            self.resolve_target(
                default_client, default_worksheet_uuid, spec, allow_remote
            )[2] for spec in target_specs
        ]
    ...
    def resolve_bundle_uuid(client, worksheet_uuid, bundle_spec):
        ...
        if spec_util.UUID_REGEX.match(bundle_spec):
            return bundle_spec
        return BundleCLI.resolve_bundle_uuids(
            client, worksheet_uuid, [bundle_spec]
        )[0]
    ...
    def print_table(
        ...
        for (i, row) in enumerate(rows):
            ...
            if i == 0:
                print >> self.stdout, indent + (
                    sum(lengths) + 2 * (len(columns) - 1)
                ) * '-'
    ...
    def get_missing_metadata(bundle_subclass, args, initial_metadata=None):
        ...
        if not initial_metadata:
            initial_metadata = {
                spec.key:
                getattr(args, metadata_util.metadata_key_to_argument(spec.key))
                for spec in bundle_subclass.get_user_defined_metadata()
            }
        return metadata_util.fill_missing_metadata(
            bundle_subclass, args, initial_metadata
        )
    ...
    def do_status_command(self, args):
        ...
        if not self.headless:
            ...
            print >> self.stdout, "server_version: %s" % worksheet_info['meta'][
                'version'
            ]
    ...
    @Commands.command(
        ...
        arguments=(
            ...
            Commands.Argument(
                '-r', '--remove', help='Remove this key.', action='store_true'
            ),
        ),
    )
    ...
    def do_download_command(self, args):
        ...
        if args.output_path:
            ...
            local_path = nested_dict_get(
                info, 'metadata', 'name', default='untitled'
            ) if subpath == '' else os.path.basename(subpath)
    ...
    def copy_bundle(
        self, source_client, source_bundle_uuid, dest_client,
        ...
        if copy_dependencies:
            ...
            for dep in source_info['dependencies']:
                self.copy_bundle(
                    source_client, dep['parent_uuid'], dest_client,
                    dest_worksheet_uuid, False, add_to_worksheet
                )
            self.copy_bundle(
                source_client, source_bundle_uuid, dest_client,
                dest_worksheet_uuid, False, add_to_worksheet
            )
            return
    ...
    def do_run_command(self, args):
        ...
        new_bundle = client.create(
            'bundles',
            self.derive_bundle(
                RunBundle.BUNDLE_TYPE, args.command, targets, metadata
            ),
            params={'worksheet': worksheet_uuid},
        )
    ...
    def do_docker_command(self, args):
        ...
        bundle_path = os.path.join(
            self.manager.codalab_home, 'local_bundles', uuid
        )
        request_network = None
        target_specs = [parse_key_target(spec) for spec in args.target_spec]
        dependencies = [
            (u'{}'.format(key), u'/{}_dependencies/{}'.format(uuid, key))
        ...
        container_id = dc.create_container(
            bundle_path, uuid, args.command, docker_image, request_network,
            dependencies, ['-it']
        )
        print >> self.stdout, '===='
    ...
    def do_ls_command(self, args):
        ...
        bundle_info_list = [
            item['bundle']
            for item in worksheet_info['items'] if item['type'] == 'bundle'
        ]
        self.print_bundle_info_list(
    ...
    def print_bundle_info_list(self, bundle_info_list, uuid_only, print_ref):
        ...
        if uuid_only:
            ...
            columns = (('ref', ) if print_ref else ()) + (
                'uuid', 'name', 'summary', 'owner', 'created', 'data_size',
                'state'
            )
            post_funcs = {
                'uuid': UUID_POST_FUNC,
            ...
            bundle_dicts = [
                {col: get(i, info, col)
                 for col in columns} for i, info in enumerate(bundle_info_list)
            ]
            self.print_table(
    ...
    def key_value_str(key, value):
        return '%-26s: %s' % (
            key,
            formatting.
            verbose_contents_str(unicode(value) if value is not None else None)
        )
    ...
    def print_basic_info(self, client, info, raw):
        ...
        # Bundle fields
        for key in (
            'bundle_type', 'uuid', 'data_hash', 'state', 'command',
            'is_anonymous'
        ):
            if not raw:
                if key not in info:
    ...
    @Commands.command(
        ...
        arguments=(
            ...
            Commands.Argument(
                '--mountpoint',
                help='Empty directory path to set up as the mountpoint for FUSE.'
            ),
            Commands.Argument(
                '--verbose',
    ...
    def print_target_info(
        self, client, bundle_uuid, subpath, head=None, tail=None
    ):
        try:
                ...
                while True:
                    ...
                    with closing(
                        client.fetch_contents_blob(
                            bundle_uuid, subpaths[i], byte_range
                        )
                    ) as contents:
    ...
    def mimic(self, args):
        ...
        for (old, new) in plan:
            print >> self.stderr, '%s => %s' % (
                self.simple_bundle_str(old), self.simple_bundle_str(new)
            )
        if len(plan) > 0:
    ...
    def worksheet_str(self, worksheet_info):
        return '%s%s%s(%s)' % (
            self.manager.session()['address'], INSTANCE_SEPARATOR,
    ...
    Item specifications, with the format depending on the specified item_type.
        ...
        bundle:    {0}
        worksheet: {1}"""
    ).format(BUNDLE_SPEC_FORMAT, WORKSHEET_SPEC_FORMAT).strip()
    ...
    def do_wedit_command(self, args):
        ...
        if args.freeze or any(
            ...
            (args.name, args.title, args.tags, args.owner_spec, args.anonymous)
        ):
            ...
            if args.file:
                if args.file == '-':
                    ...
                    with codecs.open(
                        args.file, encoding='utf-8', mode='r'
                    ) as infile:
                        lines = infile.readlines()
                lines = [line.rstrip() for line in lines]
            else:
                worksheet_info['items'] = map(
                    self.unpack_item, worksheet_info['items']
                )
    ...
    def unpack_item(item):
        ...
        value_obj = formatting.string_to_tokens(
            value
        ) if item_type == worksheet_util.TYPE_DIRECTIVE else value
    ...
    def do_wls_command(self, args):
        ...
        if args.uuid_only:
            ...
            if worksheet_dicts:
                ...
                self.print_table(
                    ('uuid', 'name', 'owner', 'permissions'), worksheet_dicts,
                    post_funcs
                )
    ...
    def do_gnew_command(self, args):
        ...
        print >> self.stdout, 'Created new group %s(%s).' % (
            group['name'], group['id']
        )
    ...
    def do_grm_command(self, args):
        ...
        print >> self.stdout, 'Deleted group %s(%s).' % (
            group['name'], group['id']
        )
    ...
    @Commands.command(
        ...
        arguments=(
            Commands.Argument(
                'group_spec',
                help='Group to show information about (%s).' %
                GROUP_SPEC_FORMAT,
                completer=GroupsCompleter
            ),
        ),
    ...
    def do_ginfo_command(self, args):
            ...
            members.append(
                {
                    ...
                    'user':
                        '%s(%s)' %
                        (group['owner']['user_name'], group['owner']['id']),
                }
            )
        ...
        print >> self.stdout, 'Members of group %s(%s):' % (
            group['name'], group['id']
        )
    ...
    def do_uadd_command(self, args):
        ...
        client.create_relationship(
            'groups', group['id'], 'admins' if args.admin else 'members',
    ...
    def print_user_info(self, user, fields=None):
        def print_attribute(key, user, should_pretty_print):
                ...
                try:
                    if key == 'time':
                        value = formatting.ratio_str(
                            formatting.duration_str, user['time_used'],
                            user['time_quota']
                        )
                    elif key == 'disk':
                        value = formatting.ratio_str(
                            formatting.size_str, user['disk_used'],
                            user['disk_quota']
                        )
                    else:
                        value = user[key]
            ...
            if should_pretty_print:
                print >> self.stdout, u'{:<15}: {}'.format(key, value
                                                          ).encode('utf-8')
            else:
                print >> self.stdout, value.encode('utf-8')

        default_fields = (
            'id', 'user_name', 'first_name', 'last_name', 'affiliation', 'url',
            'date_joined', 'last_login', 'email', 'time', 'disk'
        )
        if fields:
    ...
    def do_ufarewell_command(self, args):
        ...
        print >> self.stdout, 'Deleted user %s(%s).' % (
