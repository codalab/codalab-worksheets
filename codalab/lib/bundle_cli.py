...
class BundleCLI(object):
    ...
    def resolve_worksheet_uuid(client, base_worksheet_uuid, worksheet_spec):
        ...
        if spec_util.UUID_REGEX.match(worksheet_spec):
            worksheet_uuid = (
                worksheet_spec
            )  # Already uuid, don't need to look up specification
        else:
            worksheet_uuid = client.fetch_one(
                "worksheets",
                params={"base": base_worksheet_uuid, "specs": [worksheet_spec]},
            )["uuid"]
    ...
    WAIT_ARGUMENTS = (
        Commands.Argument(
            "-W", "--wait", action="store_true", help="Wait until run finishes."
        ),
        Commands.Argument(
            "-t",
            "--tail",
    ...
    @Commands.command(
        ...
        arguments=(
            ...
            Commands.Argument(
                "-r", "--remove", help="Remove this alias.", action="store_true"
            ),
        ),
    )
    ...
    @Commands.command(
        ...
        arguments=(
            ...
            Commands.Argument(
                "-u", "--uuid-only", help="Print only uuids.", action="store_true"
            ),
            Commands.Argument(
                "-w",
                "--worksheet-spec",
    ...
    def do_cat_command(self, args):
        ...
        if info is None:
            raise UsageError(
                "Target {} doesn't exist in bundle {}".format(subpath, bundle_uuid)
            )
    ...
    def print_target_info(self, client, bundle_uuid, subpath, head=None, tail=None):
        ...
        if info_type == "directory":
            ...
            self.print_table(
                ("name", "perm", "size"), contents, justify={"size": 1}, indent=""
            )
            ...
            for worksheet_spec in args.item_spec:
                ...
                dest_client.create(
                    ...
                    data={
                        ...
                        "subworksheet": JsonApiRelationship(
    ...
    def do_urm_command(self, args):
        ...
        member = next(
            itertools.ifilter(
                lambda m: m["id"] == user["id"], group["members"] + group["admins"]
            ),
            None,
        )
    ...
    @Commands.command(
        ...
        arguments=(
            ...
            Commands.Argument(
                "-c", "--command", dest="match_command", help="Filter by command."
            ),
