...
def get_metadata_types(cls):
    ...
    """
    return {
        spec.key: (not issubclass(spec.type, basestring) and spec.formatting)
        or spec.type.__name__
        for spec in cls.METADATA_SPECS
    }
...
def interpret_items(schemas, raw_items):
        ...
        if mode == "hidden":
            ...
            for item_index, bundle_info in bundle_infos:
                if is_missing(bundle_info):
                    blocks.append(
                        MarkupBlockSchema()
                        .load({"text": "ERROR: cannot access bundle"})
                        .data
                    )
                    continue
    ...
    def flush_worksheets():
        ...
        blocks.append(
            SubworksheetsBlock()
            .load({"subworksheet_infos": copy.deepcopy(worksheet_infos)})
            .data
        )
    ...
    for raw_index, item in enumerate(raw_items):
        ...
        try:
            ...
            is_search = (
                item_type == TYPE_DIRECTIVE and get_command(value_obj) == "search"
            )
            ...
            if item_type == TYPE_BUNDLE:
                ...
                if (
                    ...
                    blocks.append(
                        MarkupBlockSchema()
                        .load({"id": len(blocks), "text": value_obj})
                        .data
                    )
    ...
    for raw_index, interpreted_index in reversed(list(enumerate(raw_to_block))):
        if interpreted_index is None:  # e.g., blank line, directive
            ...
            interpreted_index_str = (
                str(interpreted_index[0]) + "," + str(interpreted_index[1])
            )
