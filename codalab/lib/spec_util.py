...
NAME_STR = r'[a-zA-Z_][a-zA-Z0-9_\.\-]*'
NAME_PATTERN_STR = r'[%\*a-zA-Z0-9_\.\-]+'  # Allow % for matching wildcard (SQL syntax), and * (regular expressions)
NAME_REGEX = re.compile('^' + NAME_STR + '$')  # Names (exact match)
NAME_PATTERN_REGEX = re.compile('^(' + NAME_PATTERN_STR + ')$')  # Name pattern (loose match)
NAME_PATTERN_HISTORY_REGEX = re.compile('^(' + NAME_PATTERN_STR + r')\^([0-9]*)$')
HISTORY_REGEX = re.compile(r'^\^([0-9]*)$')
ID_REGEX = re.compile('^[0-9]+$')
NOT_NAME_CHAR_REGEX = re.compile(r'[^a-zA-Z0-9_\.\-]')
HISTORY_RANGE_REGEX = re.compile(
    r'(.*\^)([0-9]+)-([0-9]+)'
)  # Allow ranges foo^1-3 => foo^1 foo^2 foo^3
BASIC_EMAIL_REGEX = re.compile(r'^[^@]+@[^@]+\.[^@]+$')
SUB_PATH_REGEX = re.compile(r'^[a-zA-Z0-9_\-.]*\Z')
...
def create_default_name(bundle_type, raw_material):
    ...
    name = re.compile(r'\-+').sub('-', name)  # Collapse '---' => '-'
