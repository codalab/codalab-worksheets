...
NAME_PATTERN_REGEX = re.compile(
    '^(' + NAME_PATTERN_STR + ')$'
)  # Name pattern (loose match)
NAME_PATTERN_HISTORY_REGEX = re.compile(
    '^(' + NAME_PATTERN_STR + ')\^([0-9]*)$'
)
...
def create_default_name(bundle_type, raw_material):
    ...
    name = (bundle_type + '-' if bundle_type else
            '') + NOT_NAME_CHAR_REGEX.sub('-', raw_material)
