...
class RunBundle(DerivedBundle):
    ...
    METADATA_SPECS.append(
        MetadataSpec(
            ...
            basestring,
            "Which docker image (either tag or digest, e.g., codalab/ubuntu:1.9) we wish to use.",
            completer=DockerImagesCompleter,
            hide_when_anonymous=True,
            default="codalab/ubuntu:1.9",
        )
    )
    METADATA_SPECS.append(
        MetadataSpec(
    ...
    METADATA_SPECS.append(
        MetadataSpec(
            "request_cpus", int, "Number of CPUs allowed for this run.", default=1
        )
    )
    METADATA_SPECS.append(
        MetadataSpec(
            "request_gpus", int, "Number of GPUs allowed for this run.", default=0
        )
    )
    METADATA_SPECS.append(
        MetadataSpec(
    ...
    METADATA_SPECS.append(
        MetadataSpec(
            "request_network", bool, "Whether to allow network access.", default=False
        )
    )
    ...
    METADATA_SPECS.append(
        MetadataSpec(
            "run_status", basestring, "Execution status of the bundle.", generated=True
        )
    )
    ...
    METADATA_SPECS.append(
        MetadataSpec("exitcode", int, "Exitcode of the process.", generated=True)
    )
    METADATA_SPECS.append(
        MetadataSpec(
            "job_handle",
