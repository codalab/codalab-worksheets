...
def checkin(worker_id):
    ...
    socket_id = local.worker_model.worker_checkin(
        ...
        request.json['cpus'], request.json['gpus'],
        request.json['memory_bytes'], request.json['dependencies']
    )

    for uuid, run in request.json['runs'].items():
        bundle = local.model.get_bundle(uuid)
        local.model.bundle_checkin(
            bundle, run, request.user.user_id, worker_id,
...
def reply(worker_id, socket_id):
    ...
    local.worker_model.send_json_message(
        socket_id, request.json, 60, autoretry=False
    )
...
def reply_data(worker_id, socket_id):
    ...
    check_reply_permission(worker_id, socket_id)
    local.worker_model.send_json_message(
        socket_id, header_message, 60, autoretry=False
    )
...
def check_run_permission(bundle):
    ...
    if not check_bundle_have_run_permission(
        local.model, request.user.user_id, bundle
    ):
...
def start_bundle(worker_id, uuid):
    ...
    if local.model.start_bundle(
        bundle, request.user.user_id, worker_id, request.json['hostname'],
...
@put(
    '/workers/<worker_id>/update_bundle_metadata/<uuid:re:%s>' %
    spec_util.UUID_STR,
...
def update_bundle_metadata(worker_id, uuid):
    ...
    allowed_keys = set(
        [
            'run_status', 'time', 'time_user', 'time_system', 'memory',
            'memory_max', 'data_size', 'last_updated', 'docker_image'
        ]
    )
    metadata_update = {}
...
def workers_info():
    ...
    for worker in data:
        ...
        running_bundles = local.model.batch_get_bundles(
            uuid=worker['run_uuids']
        )
...
def code():
    ...
    """
    response.set_header(
        'Content-Disposition', 'attachment; filename="code.tar.gz"'
    )
