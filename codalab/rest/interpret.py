...
def fetch_interpreted_worksheet(uuid):
    ...
    return 
        worksheet_info dict{}:
            ...
            items:[list] resolved items from raw_items, 
                                bundles will be grouped into table block items
            raw-list:[list] raw source lines
            raw_to_block-list:[list] raw_item to block 
                          for example, assume the first resolved item is a bundle table block that has 2 elements,
            ...
            raw_item_to_source_line: [list] raw_item to its original source line
    ...
    worksheet_info["raw_item_to_source_line"] = expanded_items_to_raw_lines
    ...
    # items: actual blocks in the worksheets, each item may group several raw items
    #        into one block i.e one table item groups multiple bundle raw items
    worksheet_info['items'] = resolve_interpreted_blocks(interpreted_blocks['blocks'])
    # raw_to_block: an array that records how raw items are grouped into items.
    # The value is of form [block_index, sub_block_index]
    worksheet_info['raw_to_block'] = interpreted_blocks['raw_to_block']
    # block_to_raw: inverse mapping of raw_to_block
