...
def fetch_interpreted_worksheet(uuid):
    ...
    Return: 
        worksheet_info dict{}:
            ...
            items:[list] resolved BLOCK items from raw_items, 
                            bundles will be grouped into table block items, text items might be grouped into one markdown block
                            TODO: This should really be renamed to block_items to avoid confusion, 
                                    but too many places are using 'item'
                                    and its not easy to tell if they all mean the same
            source-list:[list] source lines

            The following two are mainly used for setting focus on the frontend.
            raw_to_block-list:[list] raw_item to its block index pair 
                          for example, assume the first resolved block item is a bundle table that has 2 row,
            ...
            block_to_raw:[dict] Mostly a reverse mapping of raw_to_block
                            by "Mostly": for example, 
                            multiple text items may be grouped into one markdown block, so
                            the mapping skip some raw_item indices
            raw_items_to_source_index: [list] raw_item to its original source line
                            for example, if the first 5 raw_items were generated by 
                            the first source line that is a search directive,
                            then the first 5 elements in the list will be [0, 0, 0, 0, 0]
                            if the first 5 raw_items were generated by 5 source lines of bundles;
                            then the first 5 elements in the list will be [0, 1, 2, 3, 4]                                          
    """
    ...
    worksheet_info["raw_items_to_source_index"] = raw_items_to_source_index
