...
def css():
    ...
    if request.user.is_authenticated:
        return template(
            'user_authenticated_css', username=request.user.user_name
        )
    else:
...
def reset_password():
    ...
    user_info['password'] = User.encode_password(
...
def request_reset():
    ...
    if not spec_util.BASIC_EMAIL_REGEX.match(email):
        return redirect_with_query(
            '/account/changeemail', {'error': "Invalid email address."}
        )
