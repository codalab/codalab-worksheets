...
def resolve_owner_in_keywords(keywords):
    ...
    def resolve(keyword):
        ...
        if not m:
            return keyword
        return 'owner_id=%s' % getattr(
            local.model.get_user(username=m.group(1)), 'user_id', 'x'
        )
...
def get_bundle_infos(
    ...
    readable = {
        u
        for u, perm in perms.iteritems() if perm >= GROUP_OBJECT_PERMISSION_READ
    }
    anonymous = {
        u
        for u, perm in perms.iteritems() if u in bundle_infos and (
            perm < GROUP_OBJECT_PERMISSION_READ or
            bundle_infos[u]['is_anonymous']
        )
    }
    for uuid in uuids:
    ...
    if get_host_worksheets:
        ...
        worksheet_uuids = [
            uuid for l in host_worksheets.itervalues() for uuid in l
        ]
        wpermissions = local.model.get_user_worksheet_permissions(
            request.user.user_id, worksheet_uuids,
            local.model.get_worksheet_owner_ids(worksheet_uuids)
        )
        readable_worksheet_uuids = set(
            uuid for uuid, permission in wpermissions.iteritems()
...
def get_group_info(group_spec, need_admin, access_all_groups=False):
    ...
    if not is_root_user and need_admin and group_info.get('is_admin') == False:
        abort(
            httplib.FORBIDDEN, 'You are not the admin of group %s.' % group_spec
        )
    ...
    if need_admin and group_info['uuid'] == local.model.public_group_uuid:
        abort(
            httplib.FORBIDDEN, 'Cannot modify the public group %s.' % group_spec
        )
