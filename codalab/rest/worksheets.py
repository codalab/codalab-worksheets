...
def fetch_worksheet(uuid):
    ...
    if "group_permissions" in include_set:
        json_api_include(
            document, WorksheetPermissionSchema(), worksheet["group_permissions"]
        )
...
def fetch_worksheets():
    ...
    if specs:
        uuids = [
            get_worksheet_uuid_or_create(base_worksheet_uuid, spec) for spec in specs
        ]
        worksheets = [
            w.to_dict()
    ...
    if "group_permissions" in include_set:
        for w in worksheets:
            if "group_permissions" in w:
                json_api_include(
                    document, WorksheetPermissionSchema(), w["group_permissions"]
                )
...
def create_worksheets():
    ...
    worksheets = (
        WorksheetSchema(strict=True, many=True)  # only allow name for now
        .load(request.json)
        .data
    )
...
def set_worksheet_permissions():
    ...
    """
    new_permissions = (
        WorksheetPermissionSchema(strict=True, many=True).load(request.json).data
    )
