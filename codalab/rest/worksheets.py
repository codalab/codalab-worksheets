...
def fetch_worksheet(uuid):
    ...
    if user_ids:
        json_api_include(document, UserSchema(), local.model.get_users(user_ids)['results'])
...
def fetch_worksheets():
    ...
    if 'owner' in include_set:
        ...
        if owner_ids:
            json_api_include(document, UserSchema(), local.model.get_users(owner_ids)['results'])
