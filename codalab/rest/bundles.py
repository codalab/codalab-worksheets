...
def _set_bundle_permissions():
    ...
    """
    new_permissions = (
        BundlePermissionSchema(strict=True, many=True).load(request.json).data
    )
...
def _fetch_bundle_contents_blob(uuid, path=""):
    ...
    if target_info["type"] == "directory":
        ...
        if head_lines or tail_lines:
            abort(
                httplib.BAD_REQUEST, "Head and tail not supported for directory blobs."
            )
        ...
        if byte_range and (head_lines or tail_lines):
            abort(
                httplib.BAD_REQUEST, "Head and range not supported on the same request."
            )
        elif byte_range:
...
def _update_bundle_contents_blob(uuid):
    ...
    if bundle.state in State.FINAL_STATES:
        abort(
            httplib.FORBIDDEN, "Contents cannot be modified, bundle already finalized."
        )
        ...
        if len(frozen_worksheets) > 0:
            raise UsageError(
                ...
                % (
                    uuid,
                    "\n  ".join(
                        worksheet.simple_str() for worksheet in frozen_worksheets
                    ),
                )
            )
