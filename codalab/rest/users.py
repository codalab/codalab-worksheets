...
def update_authenticated_user():
    ...
    user_info = (
        AuthenticatedUserSchema(strict=True).load(request.json, partial=False).data
    )
    ...
    ) != request.user.user_name and local.model.user_exists(
    ...
    # Return updated user
    return (
        AuthenticatedUserSchema().dump(local.model.get_user(request.user.user_id)).data
    )
...
def update_users():
    ...
    users = (
        AuthenticatedUserSchema(strict=True, many=True)
