...
class ChatBoxQA(object):
    ...
    def prepare(cls):
        file_path = os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "chat_box_qa.yaml"
        )
    ...
    def get_similarity(cls, query_question, model_question):
        ...
        query_tokens = set(
            "".join(ch for ch in query_question.lower() if ch not in exclude).split()
        )
        model_tokens = set(
            "".join(ch for ch in model_question.lower() if ch not in exclude).split()
        )
        count = 0
    ...
    def get_most_similar_question_index(cls, question):
        ...
        for index in cls.qa_body:
            current_sim = cls.get_similarity(
    ...
    def answer(cls, question, worksheet_uuid, bundle_uuid):
        ...
        if custom_params == "worksheet":
            command = " ".join(
                cls.qa_body.get(index)["answer"]["command"].split()[:2]
                + [worksheet_uuid]
            )
        elif custom_params == "bundle" and bundle_uuid != "-1":
            command = " ".join(
                cls.qa_body.get(index)["answer"]["command"].split()[:2] + [bundle_uuid]
            )
        else:
