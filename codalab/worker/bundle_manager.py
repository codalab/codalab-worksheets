...
class BundleManager(object):
    ...
    def _stage_bundles(self):
        ...
        for bundle in bundles:
            ...
            if bundle.metadata.allow_failed_dependencies:
                ...
                failed_uuids = [
                    uuid
                    for uuid, state in parent_states.iteritems()
                ...
                failure_message = ""
                if failed_uuids:
                    failure_message += " Parent bundles failed: %s" % ", ".join(
                        failed_uuids
                    )
                if killed_uuids:
    ...
    def _bring_offline_stuck_running_bundles(self, workers):
        ...
        for bundle in active_bundles:
            ...
            if failure_message is not None:
                logger.info(
                    "Bringing bundle offline %s: %s", bundle.uuid, failure_message
                )
                self._model.set_offline_bundle(bundle)
    ...
    def _filter_and_sort_workers(self, workers_list, bundle):
        ...
        # in its cache.
        needed_deps = set(
            map(lambda dep: (dep.parent_uuid, dep.parent_path), bundle.dependencies)
        )
    ...
    def _construct_run_message(self, worker, bundle):
        ...
        if (
            ...
            message["bundle"]["location"] = self._bundle_store.get_bundle_location(
                bundle.uuid
            )
