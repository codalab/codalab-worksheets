...
NVIDIA_MOUNT_ERROR_REGEX = (
    r'[\s\S]*OCI runtime create failed[\s\S]*stderr:[\s\S]*nvidia-container-cli: '
    r'mount error: file creation failed:[\s\S]*nvidia-smi'
)
    ...
    def decorator(f):
        def wrapper(*args, **kwargs):
            ...
            def check_for_user_error(exception):
                ...
                logger.info("Yibo: exception format is " + str(exception))
