        ...
        for bundle in self._model.batch_get_bundles(
            ...
            failures.append(
                self._check_resource_failure(
                    ...
                    global_fail_string=
                    'No workers available with %d CPUs, max available: %d',
                    global_max=max(
                        map(lambda worker: worker['cpus'], workers_list)
                    )
                )
            )
            ...
            failures.append(
                self._check_resource_failure(
                    ...
                    global_fail_string=
                    'No workers available with %d GPUs, max available: %d',
                    global_max=max(
                        map(lambda worker: worker['gpus'], workers_list)
                    )
                )
            )
            ...
            failures.append(
                self._check_resource_failure(
                    ...
                    user_fail_string=
                    'Requested more disk (%s) than user disk quota left (%s)',
                    user_max=self._model.get_user_disk_quota_left(
                        bundle.owner_id
                    ),
                    global_fail_string=
                    'Maximum job disk size (%s) exceeded (%s)',
            ...
            failures.append(
                self._check_resource_failure(
                    ...
                    user_fail_string=
                    'Requested more time (%s) than user time quota left (%s)',
                    user_max=self._model.get_user_time_quota_left(
                        bundle.owner_id
                    ),
